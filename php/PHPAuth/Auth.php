...
require(dirname(__FILE__)."/../ZxcvbnPhp/Matchers/DigitMatch.php");
...
require(dirname(__FILE__)."/../ZxcvbnPhp/Searcher.php");
require(dirname(__FILE__)."/../ZxcvbnPhp/ScorerInterface.php");
...
class Auth
{
	...
	*/
	...
	public function register($email, $password, $repeatpassword, $account_type, $params = Array(), $captcha = NULL, $sendmail = NULL)
	{
		$return['error'] = true;
		...
		// Enable to skip password strength check
		...
		$addUser = $this->addUser($email, $password, $account_type, $params, $sendmail);
	...
	*/
	...
	private function addSession($uid, $remember)
	{
		...
		setcookie("authID", $data['hash'], time() + 1800, '/; secure; HttpOnly; samesite=none');
	...
	*/
	...
	private function addUser($email, $password, $account_type, $params = array(), &$sendmail)
	{
		$return['error'] = true;

		$query = $this->dbh->prepare("INSERT INTO {$this->config->table_users} (email, password, isactive, account_type, dt) VALUES(null, null, 0, null, now())");
		...
		$uid = $this->dbh->lastInsertId('users_id_seq');
		...
		$query = $this->dbh->prepare("UPDATE {$this->config->table_users} SET email = ?, password = ?, isactive = ?, account_type = ? {$setParams} WHERE id = ?");

		$bindParams = array_values(array_merge(array($email, $password, $isactive, $account_type), $params, array($uid)));
	...
	*/
	...
	public function getAccountType($email) {
		...
		$query = $this->dbh->prepare("SELECT account_type FROM {$this->config->table_users} WHERE LOWER(email) = LOWER(?)");
		$query->execute(array($email));
		
		$account_type = $query->fetch(\PDO::FETCH_ASSOC)['account_type'];
		return $account_type;
	}
}
